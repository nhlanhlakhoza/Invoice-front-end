...
export class Invoices4Component implements OnInit {
  ...
  applyFilter(): void {
    ...
    const urlPaid = `http://localhost:8080/user/${this.email}/payment-status/${paymentStatusPaid}`;
    ...
    this.http.get<any[]>(urlPaid).subscribe(
      paidInvoices => {
        ...
        const urlUnpaid = `http://localhost:8080/user/${this.email}/payment-status/${paymentStatusUnpaid}`;
  ...
  fetchInvoices() {
    this.http.get<any[]>(`http://localhost:8080/user/${this.email}/payment-status/unpaid`)
      .subscribe(
        invoices => {
...
loadPaidInvoices() {
  ...
  const url = `http://localhost:8080/user/${this.email}/payment-status/${paymentStatus}`;
  ...
  getInvoiceDetails(invoiceNo: number): void {
    const apiUrl = `http://localhost:8080/user/invoice/${invoiceNo}`;
